...
extern HashExtern {
    ...
    void protect(in bit<32> SAK, out bit<32> xid_field);
    void validate(in bit<32> SAK, out bit<32> valid_result);
}
...
*/    
    ...
    apply {
       if (hdr.dcp.isValid()) {
         //   dcp_lpm.apply();
         bit<32> in_sak = 0xffffffff;
         bit<32> out_xid = 0x00000000;
         h.protect(in_sak, out_xid);
         hdr.dcp.xid = out_xid;
         forward.apply();
        }
        //forward.apply();

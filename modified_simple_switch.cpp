/* Copyright 2013-present Barefoot Networks, Inc.
	...
	class ExternCrypt : public ExternType {
	 public:
	  ...
	  void protect(const Data &in_sak, Data &out_xid) {
	    std::vector<unsigned char> secure_association_key = get_char_vector(
	                                          in_sak.get_string(), XID_SIZE);
	
	    cout<<"inside protect"<<std::endl;
	    std::vector<unsigned char> user_data = secure_association_key;
	
	    std::stringstream ss_xid;
	    for (uint i = 0; i < XID_SIZE; ++i)
	        ss_xid << std::setfill('0') << std::setw(2)
	        << std::hex << static_cast<int>(user_data[i]);
	    std::string xid_hexstr = ss_xid.str();
	
	    out_xid.set(xid_hexstr);
	  }
	...
	BM_REGISTER_EXTERN_METHOD(ExternCrypt, protect, const Data &, Data &);
	BM_REGISTER_EXTERN_METHOD(ExternCrypt, validate, const Data &, Data &);
	BM_REGISTER_EXTERN_W_NAME(ext_crypt, ExternCrypt);
	BM_REGISTER_EXTERN_W_NAME_METHOD(ext_crypt, ExternCrypt, protect, const Data &, Data &);
	BM_REGISTER_EXTERN_W_NAME_METHOD(ext_crypt, ExternCrypt, validate, const Data &, Data &);
	}  // namespace bm
